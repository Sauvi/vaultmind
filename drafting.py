...
TEMPLATES = {
    ...
    "client_letter": {
        "name": "Client Letter",
        "fields": ["lawyer_name", "firm_name", "client_name", "client_address",
                   "letter_date", "subject", "matter_ref", "body_notes",
                   "governing_law"],
        "defaults": {
            "governing_law": "India",
            "matter_ref":    "General",
    ...
    "legal_notice": {
        "name": "Legal Notice",
        "fields": ["sender_name", "sender_address", "sender_lawyer",
                   "recipient_name", "recipient_address",
                   "notice_date", "subject_matter", "facts_summary",
                   "demand", "response_days", "governing_law"],
        "defaults": {
            "response_days": "15",
            "governing_law": "India",
        }
...
def draft_document(template_id: str, fields: dict) -> str:
    ...
    elif template_id == "client_letter":
        return _draft_client_letter(fields)
    elif template_id == "legal_notice":
        return _draft_legal_notice(fields)
    else:
        raise ValueError(f"Unknown template: {template_id}")
...
def _draft_client_letter(f: dict) -> str:
    lawyer     = f.get("lawyer_name",   "[Lawyer Name]")
    firm       = f.get("firm_name",     "[Firm Name]")
    client     = f.get("client_name",   "[Client Name]")
    address    = f.get("client_address","[Client Address]")
    date       = _format_date(f.get("letter_date", ""))
    subject    = f.get("subject",       "[Subject of letter]")
    matter_ref = f.get("matter_ref",    "[Matter Reference]")
    notes      = f.get("body_notes",    "[Key points / instructions for this letter]")
    gov_law    = f.get("governing_law", "India")

    note_lines      = [n.strip() for n in notes.split("\n") if n.strip()]
    body_paragraphs = "\n\n".join(note_lines) if note_lines else notes

    return f"""[ON FIRM LETTERHEAD]
================================================================================
...
We act as legal counsel in the above-captioned matter and write to update you
as follows.

{body_paragraphs}

Please review the above and revert with your instructions at the earliest.
Should you have any questions or require clarification on any of the points
raised above, please do not hesitate to contact this office.
...
================================================================================
This letter is governed by the laws of {gov_law}.
DRAFT — Generated by VaultMind. Review and customise before sending.
================================================================================"""


def _draft_legal_notice(f: dict) -> str:
    sender         = f.get("sender_name",       "[Sender Full Name]")
    sender_addr    = f.get("sender_address",    "[Sender Address]")
    sender_lawyer  = f.get("sender_lawyer",     "[Advocate Name & Bar No.]")
    recipient      = f.get("recipient_name",    "[Recipient Full Name / Company]")
    recipient_addr = f.get("recipient_address", "[Recipient Address]")
    date           = _format_date(f.get("notice_date", ""))
    subject        = f.get("subject_matter",    "[Brief subject — e.g. Non-payment of dues]")
    facts          = f.get("facts_summary",     "[State the facts chronologically]")
    demand         = f.get("demand",            "[State what you demand]")
    days           = f.get("response_days",     "15")
    gov_law        = f.get("governing_law",     "India")

    facts_lines = [l.strip() for l in facts.split("\n") if l.strip()]
    facts_block = "\n".join(f"   {i+1}. {line}" for i, line in enumerate(facts_lines)) \
                  if facts_lines else f"   {facts}"
...
{sender}
{sender_addr}
Through Advocate: {sender_lawyer}
...
Sub: Legal Notice — {subject}

================================================================================

NOTICE

Under the instructions and on behalf of my client, {sender} (hereinafter
referred to as "my client"), I hereby serve upon you this Legal Notice as
follows:
...
The above acts / omissions of the Noticee constitute a breach of the agreement
/ legal obligation between the parties and give rise to a cause of action in
favour of my client under the applicable laws of {gov_law}, including but not
limited to the Indian Contract Act, 1872 (and/or any other applicable statute
as may be relevant).
...
You are hereby called upon to comply with the above demand within {days} days
...
competent courts / forums at your risk, cost, and consequences.

5. RESERVATION OF RIGHTS

All rights of my client are expressly reserved, including the right to claim
damages, interest, costs, and any other relief as may be deemed appropriate.

This Notice is issued without prejudice to all other rights and remedies
